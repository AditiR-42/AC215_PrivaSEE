...
from api_service.api.routers.recommend import filter_dataframe, GenerativeModel
import pandas as pd
...
def sample_df():
    return pd.DataFrame({
        ...
        'Genre': ['Social', 'Music', 'Productivity'],
        'privacy_rating': ['A', 'B', 'C'],
        'Content Rating': ['Everyone', 'Teen', 'Mature'],
        'app_score': [4.5, 4.0, 3.5],
        'Installs': [1000000, 500000, 100000],
        'num_ratings': [10000, 5000, 1000],
        'num_reviews': [5000, 2500, 500],
        'Free': [True, False, True],
        'Contains Ads': [True, False, True]
    })

@pytest.fixture
def mock_model():
    mock = MagicMock(spec=GenerativeModel)
    mock.generate_content.return_value = MagicMock(text="social")
    return mock
...
def test_service_criterion_updates_genre_and_privacy_rating(sample_df):
    with patch('your_module.df', sample_df):
        ...
        result = filter_dataframe(criteria)
        assert len(result) == 1
        assert result.iloc[0]['Genre'] == 'Music'
        assert result.iloc[0]['privacy_rating'] == 'B'
